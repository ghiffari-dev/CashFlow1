    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4 rounded-2xl bg-surface p-4 shadow-sm">
        ...
        <div>
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">
            Untuk hasil terbaik gunakan foto portrait (rasio 4:5). Foto tidak diunggah — tetap di
            perangkatmu.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="rounded-2xl bg-surface p-3 shadow-sm">
          <div className="aspect-[4/5] w-full overflow-hidden rounded-xl bg-background">
            <div
              style={{
                width: 1080,
                height: 1350,
...
function ScaleObserver() {
  useEffect(() => {
    const set = () => {
      const el = document.querySelector<HTMLDivElement>(".aspect-\\[4\\/5\\]");
      if (!el) return;
      const w = el.clientWidth;
      const scale = w / 1080;
      el.parentElement?.style.setProperty("--recap-scale", String(scale));
...
async function renderRecapVideo(node: HTMLElement): Promise<Blob> {
  ...
  // Snapshot the card once at 1080x1350
